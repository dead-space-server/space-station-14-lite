...
namespace Content.Server.Preferences.Managers
{
    ...
    public sealed class ServerPreferencesManager : IServerPreferencesManager, IPostInjectInit
    {
        ...
        public async Task SetProfile(NetUserId userId, int slot, ICharacterProfile profile)
        {
            ...
            var allowedMarkings = _sponsors.TryGetInfo(session.Channel.UserId, out var sponsor) ? sponsor.AllowedMarkings.ToArray() : [];
        ...
        public async Task LoadData(ICommonSession session, CancellationToken cancel)
        {
            ...
            else
            {
                ...
                async Task LoadPrefs()
                {
                    ...
                    {
                        var allowedMarkings = _sponsors.TryGetInfo(session.UserId, out var sponsor) ? sponsor.AllowedMarkings.ToArray() : [];
        ...
        private PlayerPreferences SanitizePreferences(ICommonSession session, PlayerPreferences prefs, IDependencyCollection collection)
        {
            ...
            var allowedMarkings = _sponsors.TryGetInfo(session.UserId, out var sponsor) ? sponsor.AllowedMarkings.ToArray() : []; // Corvax-Sponsors

            return new PlayerPreferences(prefs.Characters.Select(p =>
            {
                return new KeyValuePair<int, ICharacterProfile>(p.Key, p.Value.Validated(session, collection, allowedMarkings)); // Corvax-Sponsors
